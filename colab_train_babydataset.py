...
class EmergencyOnlyCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if logs and 'loss' in logs:
            ...
            if self.logger:
                self.logger.info(
                    f"步数 {state.global_step}: loss = {loss:.4f}, 困惑度 = {perplexity:.4f}")
    ...
    def _save_final_model(self, state):
        ...
        try:
            ...
            with open(os.path.join(final_model_path, "training_results.json"), 'w') as f:
...
def merge_dataset_files(data_dir, file_pattern="*.train"):
    """合并指定目录下的所有训练文件"""
    import glob
    ...
    if not data_files:
        raise FileNotFoundError(f"在目录 {data_dir} 中没有找到训练文件")
    ...
    print(f"从 {total_files} 个文件中读取了 {total_lines} 行数据")

    # 创建临时合并文件
    temp_file = os.path.join(data_dir, "merged_data.tmp")
    with open(temp_file, 'w', encoding='utf-8') as f:
        f.writelines(all_lines)

    print(f"数据已合并到临时文件: {temp_file}")

    return temp_file, total_lines
...
def train_with_emergency_only(
    ...
    logger.info(f"数据目录: {dataset_config['data_dir']}")
    ...
    # 检查数据目录是否存在
    if not os.path.exists(dataset_config['data_dir']):
        logger.error(f"数据目录不存在: {dataset_config['data_dir']}")
        logger.info("请确保数据目录已放置在正确位置")
        logger.info(f"数据目录应该位于: {dataset_config['data_dir']}")
        return None, None

    # 合并数据集文件
    try:
        temp_file, total_lines = merge_dataset_files(
            dataset_config['data_dir'],
            dataset_config.get('file_pattern', "*.train")
            )
        logger.info(f"数据合并完成，共 {total_lines} 行")
    except Exception as e:
            ...
            if choice.lower() != 'y':
                ...
                try:
                    result_file = os.path.join(
                        final_model_path, "training_results.json")
                    with open(result_file, 'r') as f:
                        existing_results = json.load(f)
                    return {"losses": existing_results.get("losses", []),
                            "perplexities": existing_results.get("perplexities", [])}, existing_results
                except BaseException:
    ...
    training_args = TrainingArguments(
        ...
        adam_epsilon=1e-8,
        max_grad_norm=1.0,
        max_steps=config.MAX_STEPS,  # 使用配置中的MAX_STEPS (2000)
        warmup_steps=100,
    ...
    try:
        ...
        logger.info(f"5. 训练步数: {config.MAX_STEPS}")
        logger.info(f"6. 如果训练中断，可以从最新紧急存档恢复")
        logger.info("=" * 60)
        ...
        # 收集训练指标
        losses = emergency_callback.losses
        perplexities = emergency_callback.perplexities

        logger.info(f"训练完成: {model_name}")
        if losses and perplexities:
            logger.info(
                f"最终loss: {losses[-1]:.4f}, 最终困惑度: {perplexities[-1]:.4f}")
            logger.info(
                f"最小loss: {
                    min(losses):.4f}, 最小困惑度: {
                    min(perplexities):.4f}")
            if len(losses) > 1:
                logger.info(f"loss下降: {(losses[0] - losses[-1]):.4f}")
                logger.info(
                    f"困惑度下降: {(perplexities[0] - perplexities[-1]):.4f}")
        ...
        return {"losses": losses, "perplexities": perplexities}, train_result
...
def main():
    ...
    print(f"训练步数: {TrainingConfig.MAX_STEPS}")
    print("只保留紧急存档")
    print("=" * 80)
    ...
    logger.info(f"训练步数: {config.MAX_STEPS}")
    ...
    for dataset_config in config.DATASETS:
        ...
        # 训练模型
        training_data, train_result = train_with_emergency_only(
    ...
    for dataset_name, metrics in all_metrics.items():
        if metrics:
            ...
            print(f"   最终loss: {final_loss:.4f}, 最终困惑度: {final_ppl:.4f}")
            print(f"   最小loss: {min_loss:.4f}, 最小困惑度: {min_ppl:.4f}")
            print(f"   训练步数: {steps}")
    ...
    if IS_COLAB:
        ...
        for dataset in config.DATASETS:
            ...
            print(f"   │   └── final_model/ (最终模型和训练结果)")
    else:
        print("\n📊 训练状态汇总:")
        print(f"   日志文件: {config.LOG_FILE}")
        print(f"   所有模型保存在: {config.RESULTS_DIR}")
        print(f"   数据目录结构: {config.DATA_DIR}/")
        print(f"   ├── natural/ (自然语言)")
        print(f"   ├── reversed/ (反转)")
        print(f"   └── parity_negation/ (奇偶否定)")
