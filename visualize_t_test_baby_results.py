...
def visualize_t_test_results(data, output_dir):
    """
    Visualize Welch's t-test results for baby dataset with grouped bar charts.
    """
    # Set style
    ...
    plt.tight_layout()
    loss_output = os.path.join(output_dir, 't_test_baby_loss_comparison.png')
    ...
    bars1 = ax.bar(x - width/2, means_group1, width, yerr=stds_group1,
                   capsize=5, label=test['group1']['name'],
                   color='#e67e22', alpha=0.8, error_kw={'elinewidth': 1.5})
    bars2 = ax.bar(x + width/2, means_group2, width, yerr=stds_group2,
                   capsize=5, label=test['group2']['name'],
                   color='#34495e', alpha=0.8, error_kw={'elinewidth': 1.5})

    # Add significance labels
    for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
        max_height = max(bar1.get_height(), bar2.get_height())
        max_std = max(stds_group1[i], stds_group2[i])
        sig_label = '***' if perplexity_tests[i]['significance'] == 'significant' else 'ns'
        ax.text(i, max_height + max_std + 1, sig_label,
                ha='center', va='bottom', fontsize=14, fontweight='bold')

    # Customize plot
    ax.set_xlabel('Comparison', fontsize=12, fontweight='bold')
    ax.set_ylabel('Perplexity Value', fontsize=12, fontweight='bold')
    ax.set_title("Welch's T-Test Results: Perplexity Comparisons (Baby Dataset)",
                 fontsize=14, fontweight='bold', pad=20)
    ax.set_xticks(x)
    ax.set_xticklabels(groups, fontsize=10)
    ax.legend(fontsize=11, loc='upper right')
    ...
    plt.tight_layout()
    perplexity_output = os.path.join(output_dir, 't_test_baby_perplexity_comparison.png')
    ...
    ax_loss.set_title('Loss Tests Summary (Baby Dataset)', fontsize=14, fontweight='bold', pad=10)
    ...
    for i in range(len(perp_table_data[0])):
        table2[(0, i)].set_facecolor('#1abc9c')
        table2[(0, i)].set_text_props(weight='bold', color='white')

    ax_perp.set_title('Perplexity Tests Summary (Baby Dataset)', fontsize=14, fontweight='bold', pad=10)

    plt.tight_layout()
    table_output = os.path.join(output_dir, 't_test_baby_summary_table.png')
...
def main():
    ...
    json_file = os.path.join(base_dir, 'statistics', 'welch_t_test_baby_results.json')
    ...
    # Visualize
    print("\nGenerating visualizations for baby dataset...")
