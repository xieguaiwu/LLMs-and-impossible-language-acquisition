...
### 10c-13. Positive-evidence arms for architectural inductive bias (registered 2026-09-24, before any new cell)

Status: registered ex ante. No data for the new arms existed at registration time.
Source: research/DESIGN_refined_transformer_bias_20260924.md (this document).

(Naming: the new analysis families use the G-namespace — G1_ppd / G2_cald / G3_ags — disjoint from
 STATS_PLAN F1–F5 (Holm), the §10c-5/6 registered families F8_datascale/F9_model_scale, and the
 FALSIFICATION F1–F10 numbering.)

A. Arms added (all GPT-2-small / BabyLM protocol verbatim unless stated)
  A1. nope_ext: GPT2_NOPE=1, conditions {fixed_start, not_random} × seeds {0,14,41} = 6 cells.
      (base nope arm §10c-4 stays as registered: {parity_word, shuffle_control} × 3 = 6 cells;
      SMOKE GATE: when the first base-NoPE cell lands, the content-penalty is measured immediately;
      if it exceeds 0.15 nats, T1 is declared "uninterpretable" ex ante and the nope_ext cells are
      re-purposed per §4.3 contraction, freeing 23.7 GPU·h)
  A2. cald: new synthetic condition family generated by design_v3/make_cald_conditions.py:
      cald_local, cald_long, cald_shuf (V ⊥ K) × seeds {0,14,41} = 9 adjudicated cells, plus 2 pilot
      cells (cald_long, cald_shuf @ seed 0) that are DISCARDED after calibration and excluded from
      adjudication; grid_status expected counts include the 2 discarded cells (11 total).
      Data dirs cald_<cond>/, REPRO_DIR_TAG=_cald, results tree results_cald/, publish branch
      v2-results-cald.
      Generator (frozen): fillers sampled from the BabyLM natural BIGRAM transition distribution
      (local dependency is required for induction-head formation, Aoyama et al. 2025; identical
      sampling process for all three conditions; cald_shuf permutes only the V assignment);
      K/V tokens disjoint from fillers; K=200 pairs, d~U{8,32}, len 24-48 tokens.
      Surface-match gate (construction-time): content column |Δ ln ppl| ≤ 0.15 across all three
      conditions (bigram and 5-gram); all column constrained only for long↔shuf (local's adjacent
      (K,V) bigram is a pre-declared constructive exception); pre-declared resampler on failure,
      iterations disclosed.
      Primary metric: V-token ppl computed offline by probes/cald_value_ppl.py on final/ weights
      (NLL of V tokens given the full prefix).
      Calibration protocol: the ONLY tunable is one scalar (d-range or K); selection rule = GPT-2
      final V-token ppl in [2,20]; the chosen value is registered by an AMENDMENT §10c-13a (with
      full pilot disclosure) before any confirmatory cell is queued; all pilot runs disclosed.
  A3. t5_seed_ext: not_random × seeds {53,96} = 2 cells (completes the position-matched pair to n=5).
  A4. (Tier 2, only if budget allows, registered now to avoid post-hoc selection)
      cald_nest2 / cald_ser2 × 3 seeds = 6 cells; CALD GPU-LSTM subset
      {cald_local, cald_long, cald_shuf} × 3 seeds = 9 cells at matched token budget (3.93e8);
      n=5 extensions for cald_* and nope_ext = 12 cells.

B. Deferrals (frees ~95 GPU-h; each is a REGISTERED DEVIATION with its own §10 log entry keyed
  to the original §10c item — explicit logged priority change, not a silent edit):
  logo (2 cells; deviation log §10c-13 DL-1), model_scale (6→2 cells; §10c-13 DL-2, keys §10c-6/FC6,
  2-point descriptive), datascale (12→4 cells; §10c-13 DL-3, keys §10c-5/FC5 — keeps parity_word &
  shuffle_control × sub1M × seeds 0/14; sub10M dropped ⇒ FC5 dose-response reduced to endpoint,
  disclosed), ladder_probe replay (2→0 cells; §10c-13 DL-4, keys §10c-3 — all P-class cells already
  embed LADDER_PROBE=1).
  Rationale: all four are exploratory/BH-bucket; FC5/FC6 keep reduced descriptive signals.

C. Analysis (criterion-based; no alpha spent except where noted)
  G1_ppd (T1): P1a |Δ_NoPE| ≤ 0.5|Δ_wpe| both branches (fixed_start both branches + parity_word
      obey_last), ≥2/3 seeds; P1b content penalty ≤0.15 nats per condition, max over
      {fixed_start, parity_word, not_random}; P1b-N marker-token NLL column reported;
      P1c NoPE penalty(fixed_start) − penalty(parity_word) ≥ 0.30 nats (all-token); P1d shape
      criteria (fixed_start monotone rho≥0.8; parity_word U-shape mid−ends ≥0.2; NoPE flat
      |Δ(k)|<0.2); P1e not_random prior magnitude between fixed_start and parity_word, zeroed
      under NoPE. T1 PRIMARY = P1a AND P1b (P1b is the interpretability gate).
  G2_cald (T2): CALD1 = [ln ppl_V(shuf) − ln ppl_V(long)]_GPT2 ≥ 0.5 nats with the same floor
      difference ≤ 0.1 nats, 3/3 seeds; CALD2 AG(long) − AG(local) ≥ 0.5; CALD3 distance-bucket
      spread ≤ 0.3; CALD4 (Tier2) LSTM gap < GPT-2 gap; CALD5 |nest2 − ser2| ≤ 0.3.
  G3_ags / MSSC (T5; criterion-based, NO p-values computed — BH not applicable; family-level
      control = frozen pair list + pre-registered directions):
      AGS1 P-class max−min AG ≤ 0.15 nats (expected: surface-reducible);
      AGS2 DESCRIPTIVE EXPECTATION (reversed 2026-09-24 per review P0-5; old-code seed0:
      AG(nondet)=1.029 < AG(local3)=1.457): AG(shuffle_local3) − AG(shuffle_nondeterministic)
      ≥ 0.15 nats, re-verified on new-code cells; descriptive, not a gate;
      MSSC pairs frozen by floor only (|Δln ppl_floor| ≤ 0.15, all and content; list in the
      design doc §3.5);
      MSSC1 ≥1 pair |DoD| ≥ 0.15 nats and 3/3 seed sign consistency; MSSC2 R-pair
      (A = reverse_partial, B = reverse_control) DoD ≥ 0.15; MSSC3 parity pair three-segment:
      |DoD| ≤ 0.10 pass (pre-declared null), > 0.15 fail (partial learning; route through
      F1/H10), 0.10–0.15 = not adjudicable at n=3; MSSC4 any content-matched pair DoD ≥ 0.15;
      General n=3 boundary category: 3/3 sign consistency with sub-threshold magnitude is
      reported as SUGGESTIVE and licenses no claim.
  Mechanism criteria (T3/T4, criterion-based, no alpha):
      GPP1 two-tier: primary = positive direction + ≥60% items same sign + floor gap ≤0.3;
      upper tier = gap ≥1.5 nats (strong-effect wording only if met; sub-tier results reported
      as effect sizes, not failures);
      GPP2 DoD(far−near) ≥ 0.10 nats (≥60% pairs; primary = seed0, seeds 14/41 as sensitivity);
      IHC1 ≥1 head prefix-match >0.30 in ≥2/3 seeds; IHC2 far−near interaction ≥0.20 nats
      (paired, one-sided, 3/3 seeds) with random-k |interaction| ≤0.05; IHC3 whole-test ppl
      increase ≤5%.

D. Contraction rules (pre-registered): see design doc §4.3. The strong claim
   ("structural/computational inductive bias with non-surface advantage") is licensed only if
   BOTH G2_cald primary (CALD1) and G1_ppd primary (P1a ∧ P1b) criteria pass. Otherwise the
   pre-written weaker wording applies; if both T2 and T5 fail, premise (a) of the Chomskyan
   critique is recorded as "not falsified at this budget", and the contribution is limited to
   erratum + methodology + formal-complexity mapping.
   FC2↔MSSC3 reconciliation: MSSC3 IS the FC2 adjudication (counting cost beyond surface);
   both outcomes have pre-written wording (pass ⇒ surface-explained; fail ⇒ partial learning
   routed through F1/H10).

E. Hygiene: no change to any running arm; queue-order change (nope moved earlier) executed at a
   cell boundary and logged; grid_status.py updated with the new expected counts; all new
   conditions generated by the single-source generator with a manifest; dry-run gate before
   queue rebuild; results published on separate branches.
B2. Deferral deviation log (each entry keys the deferral to its originally registered item;
    logged priority changes, operator approval 2026-09-24):
    DL-1 (logo): the 2 registered logo cells (domain-transfer arm) are deferred, not cancelled;
        the domain-transfer question is answered by T3/T4 data-side contrasts at zero training
        cost. FC impact: none (logo was not an FC carrier).
    DL-2 (model_scale): 6 -> 2 cells (shuffle_control/parity_word x seed0 only; FC6). FC6 keeps
        a 2-point descriptive signal; seeds 14/41 and fixed_start deferred to Tier-3.
    DL-3 (datascale): 12 -> 4 cells (parity_word & shuffle_control x sub1M x seeds 0/14; FC5).
        sub10M dropped => FC5 dose-response reduced to a single-dose endpoint, disclosed as a
